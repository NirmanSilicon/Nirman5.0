...
export default function Pharmacies() {
  ...
  const [results, setResults] = useState<PharmacyItem[]>([]);
  ...
  const manualSearch = async () => {
    ...
    const pharmacies = await fetchNearby(coords.lat, coords.lng, "pharmacy");
    const withDistance = pharmacies.map((p) => {
      const dist =
        p.lat && p.lng
          ? haversineDistance(coords.lat, coords.lng, p.lat, p.lng)
          : null;
      return {
        ...p,
        distance: dist
  ...
  const detectNearbyPharmacies = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        const pharmacies = await fetchNearby(latitude, longitude, "pharmacy");
        const withDistance = pharmacies.map((p) => {
          const dist =
            p.lat && p.lng
              ? haversineDistance(latitude, longitude, p.lat, p.lng)
              : null;
          return {
            ...p,
            distance: dist
  ...
  // Directions & Call
  // -----------------------
  const handleGetDirections = (pharmacy: PharmacyItem) => {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center px-4 py-2 rounded-full bg-primary/10 text-primary mb-4">
            <Building2 className="h-4 w-4 mr-2" /> Pharmacy Directory
          </div>
          <h1 className="text-4xl font-bold mb-2">Find Pharmacies Near You</h1>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Locate nearby pharmacies with available medicines and services.
          </p>
        </div>
        ...
        <div className="max-w-md mx-auto mb-8">
          ...
          <div className="flex gap-3 mt-3 justify-center">
            ...
            <Button variant="outline" onClick={detectNearbyPharmacies}>
              📍 Use My Location
            </Button>
          </div>
        </div>
              ...
              >
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {pharmacy.distance && (
                      <Badge className="bg-primary/10 text-primary text-xs">{pharmacy.distance}</Badge>
                    )}
                  </div>
