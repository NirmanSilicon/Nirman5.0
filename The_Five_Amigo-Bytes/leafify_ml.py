...
def apply_gradcam_high_accuracy(model, img_path, transform, device, layer_name='conv_block5', threshold=0.4):
    ...
    if target_layer is None:
        print(f"❌ Error: Could not find layer. Check model architecture.")
        return
...
try:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("⚠️ Using ResNet50 for demonstration (Replace with your own model)")
...
def interactive_disease_diagnosis(model_path, dataset_dir):
    ...
    if num_classes == 0:
        print("❌ No classes found. Check dataset path.")
        return
    ...
    fig.suptitle("🌿 AI Plant Disease Diagnosis", fontsize=24, fontweight='bold', y=0.98, color='green')
